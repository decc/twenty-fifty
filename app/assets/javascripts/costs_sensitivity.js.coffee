class CostsSensitivity
  ...
  drawChart: () ->
    ...
    y = d3.scale.ordinal().domain(['p','i','c']).rangeRoundBands([30,180],0.15)
    ...
    r.text(30,y('c')+9,twentyfifty.pathwayName(c._id,c._id)).attr({'text-anchor':'start','font-weight':'bold', href: twentyfifty.url({action:'primary_energy_chart',code:c._id})})
    r.text(30,y('c')+27,twentyfifty.pathwayDescriptions(c._id,"")).attr({'text-anchor':'start'})
