class CostsInContext
  ...
  setupComparisonChart: () =>
    ...
    for code in ["chosen"].concat(twentyfifty.comparator_pathways)
      ...
      @boxes_low[code] = @r.rect(@x(0),@y(code),0,@y.rangeBand()/2).attr({'fill':'#134B9F','stroke':'none'})
      @boxes_range[code] = @r.rect(@x(0),@y(code)+@y.rangeBand()/2,0,@y.rangeBand()/2).attr({'fill':'#60A4FA','stroke':'none'})
  ...
  updateBar: (pathway,_id = pathway._id) =>
    ...
    @boxes_range[_id].attr({x:@x(0),width: @x(total_range+total_cost) - @x(0)})
#     yrealtop = @y(_id)
#     yband = @y.rangeBand()
